...
type PricingMode = "per_pound" | "per_unit" | "starting_from";
...
const CATEGORIES: ProductCategory[] = [
	{
		id: "ropa",
		label: "Ropa, zapatos, bolsos y documentos",
		type: "standard",
		pricingMode: "per_pound",
		price: PRICING.standard.price,
		priceLabel: "$10/lb",
		description: "Categoria base para envios generales.",
	},
	{
		id: "otros",
		label: "Otros productos generales",
		type: "standard",
		pricingMode: "per_pound",
		price: PRICING.standard.price,
		priceLabel: "$10/lb",
		description: "Articulos que no entran en categorias especiales.",
	},
	{
		id: "skincare",
		label: "Skincare",
		...
		priceLabel: "$15/lb",
		description: "Cuidado de piel con manejo especial.",
	},
	{
		id: "perfumes",
		label: "Perfumes",
		...
		price: PRICING.premium.price,
		priceLabel: "$15/lb",
		description: "Fragancias y colonias.",
	},
	{
		id: "lociones",
		label: "Lociones",
		...
		price: PRICING.premium.price,
		priceLabel: "$15/lb",
		description: "Lociones corporales y de cuidado personal.",
	},
	{
		id: "vitaminas",
		label: "Vitaminas",
	...
	},
	{
		...
		price: PRICING.premium.price,
		priceLabel: "$15/lb",
		description: "Shampoo y productos capilares.",
	},
	{
		id: "cremas",
		label: "Cremas",
		...
		price: PRICING.premium.price,
		priceLabel: "$15/lb",
		description: "Cremas de uso cosmetico o dermatologico.",
	},
	{
		id: "suministros_medicos",
		label: "Suministros medicos",
		type: "premium",
		pricingMode: "per_pound",
		price: PRICING.premium.price,
		priceLabel: "$15/lb",
		description: "Insumos medicos permitidos para courier.",
	},
	{
		id: "iphone",
		label: "iPhone",
		...
		price: PRICING.iphone.price,
		priceLabel: "$100 por iPhone",
		description: "Tarifa por unidad.",
		note: "Puede bajar si el iPhone es antiguo o de menor gama.",
	},
	{
		id: "repuestos",
		label: "Repuestos",
		...
		price: PRICING.spareParts.price,
		priceLabel: "Desde $20",
		description: "Repuestos para cualquier tipo de producto.",
		note: "El precio final depende del tipo de repuesto y tamano.",
	},
];
...
export function Quoter() {
	...
	const isPerPoundCategory = selectedCategory.pricingMode === "per_pound";
	const parsedWeight = Number.parseFloat(weight);
	const parsedQuantity = Number.parseInt(quantity, 10);
	const canAddProduct = isPerPoundCategory
	...
	const totalUnits = products.reduce(
		(sum, p) => sum + (p.pricingMode === "per_pound" ? 0 : p.quantity),
		0,
	);
	...
	const fetchAmazonWeight = async (url: string) => {
		...
		try {
			...
			if (data.success && data.data?.weight) {
				...
				if (
					data.data.suggestedCategory &&
					CATEGORIES.some((c) => c.id === data.data.suggestedCategory)
				) {
				...
				const categoryLabel = data.data.suggestedCategory
					...
					: null;
				setFetchMessage(
					`Peso: ${data.data.weightRaw}${categoryLabel ? ` | Categoria: ${categoryLabel}` : ""}`,
				);
			} else {
				setFetchMessage(
					data.message || "No se encontro el peso. Ingresalo manualmente.",
				);
				if (data.data?.title) {
					setProductName(data.data.title);
				}
	...
	const handleLinkChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (
			url.length > 20 &&
			(url.includes("amazon.com/") || url.includes("amzn."))
		) {
	...
	const handleAddProduct = () => {
		...
		const weightNum = isPerPoundCategory ? Number.parseFloat(weight) : 0;
		const quantityNum =
			selectedCat.pricingMode === "per_pound"
				? 0
		...
		const total =
			selectedCat.pricingMode === "per_pound"
				? weightNum * selectedCat.price
				: quantityNum * selectedCat.price;

		const newProduct: QuotedProduct = {
			id: `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
			name:
				productName ||
				(amazonLink ? "Producto de Amazon" : "Producto personalizado"),
			...
			pricingMode: selectedCat.pricingMode,
			price: selectedCat.price,
			priceLabel: selectedCat.priceLabel,
			totalPrice: total,
		};
   ...
   - Categoria: ${p.categoryLabel}
	   - Peso: ${p.weight.toFixed(2)} lb
	   - Cantidad: ${p.pricingMode === "per_pound" ? "N/A" : p.quantity}
	   - Tarifa: ${p.priceLabel}
...
- Unidades especiales: ${totalUnits}
			...
			<Container>
				<div className="text-center mb-12 animate-fade-in-up">
					...
					<p className="text-lg text-muted-foreground max-w-2xl mx-auto">
						Pega el link de tu producto de Amazon, selecciona la categoria y
						obtene un estimado del costo.
					</p>
				</div>

				{/* Pricing Cards */}
				<div className="max-w-3xl mx-auto mb-8 grid sm:grid-cols-2 gap-4 animate-fade-in-up">
					<div className="bg-[#3B9AC4]/10 border-2 border-[#3B9AC4]/30 rounded-2xl p-4">
						<p className="text-2xl font-bold italic text-[#3B9AC4]">
							{PRICING.standard.label}
						</p>
						<p className="text-sm text-muted-foreground mt-1">
							Ropa, zapatos, bolsos, documentos y otros productos generales.
						</p>
					</div>
					<div className="bg-[#F9A826]/10 border-2 border-[#F9A826]/30 rounded-2xl p-4">
						<p className="text-2xl font-bold italic text-[#F9A826]">
							{PRICING.premium.label}
						</p>
						<p className="text-sm text-muted-foreground mt-1">
							Skincare, perfumes, lociones, vitaminas, shampoo, cremas y
							suministros medicos.
						</p>
					</div>
					<div className="bg-emerald-100/60 border-2 border-emerald-300/50 rounded-2xl p-4">
						<p className="text-2xl font-bold italic text-emerald-700">
							{PRICING.iphone.label}
						</p>
						<p className="text-sm text-muted-foreground mt-1">
							Tarifa por unidad. Puede bajar para modelos antiguos o de menor
							gama.
						</p>
					</div>
					<div className="bg-orange-100/60 border-2 border-orange-300/50 rounded-2xl p-4">
						<p className="text-2xl font-bold italic text-orange-700">
							{PRICING.spareParts.label}
						</p>
						<p className="text-sm text-muted-foreground mt-1">
							Repuestos de cualquier rubro. El precio final depende del tipo de
							pieza.
						</p>
					</div>
				</div>
						...
						<CardContent className="p-6 md:p-8">
							<div className="space-y-6">
								<div>
									...
									<Input
										type="text"
										placeholder="Ej: 2 cremas faciales de 250 ml"
								...
								<div>
									...
									{!isValidAmazonLink(amazonLink) && (
										<p className="text-red-500 text-sm mt-1">
											Por favor ingresa un link valido de Amazon
										</p>
									)}
									{fetchMessage && (
										<p
											className={`text-sm mt-1 ${fetchMessage.includes("encontrado") ? "text-green-600" : "text-muted-foreground"}`}
										>
											{fetchMessage}
										</p>
									)}
								...
								<div>
									...
									<div className="mt-3 rounded-xl border border-border bg-muted/40 p-3">
										<p className="text-sm font-semibold text-foreground">
											Tarifa seleccionada: {selectedCategory.priceLabel}
										</p>
										<p className="text-sm text-muted-foreground mt-1">
								...
								{isPerPoundCategory ? (
									<div>
										...
										<p className="text-muted-foreground text-sm mt-1">
											Puedes encontrar el peso en la descripcion del producto en
											Amazon.
										</p>
									</div>
								) : (
									<div>
										...
										/>
										<p className="text-muted-foreground text-sm mt-1">
											{selectedCategory.pricingMode === "per_unit"
												? "Tarifa fija por cada iPhone."
												: "Precio base por unidad de repuesto."}
										</p>
									</div>
								)}
									...
									<div className="mt-6 p-6 bg-gradient-to-r from-[#3B9AC4]/10 to-[#F9A826]/10 rounded-2xl border border-[#3B9AC4]/30 animate-fade-in-up">
										<div className="text-center">
											...
											<p className="text-sm text-muted-foreground mb-4">
												...
												{products.length > 1 ? "s" : ""}
												{totalWeight > 0
													? ` | ${totalWeight.toFixed(2)} lb total`
													: ""}
												{totalUnits > 0 ? ` | ${totalUnits} unidad(es)` : ""}
											</p>
